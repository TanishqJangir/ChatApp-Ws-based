  ...
  return (
    <>
      <div className='h-screen bg-black w-screen flex flex-col justify-center'>
        ...
        <div className='h-[5%] flex justify-center gap-1'>
          <input type='text'
            ref={messageRef}
            placeholder='enter text'
            className='text-2xl text-white pl-4 rounded-md border-2 outline-none w-[80%]'
            onKeyDown={(e) => {
              if (e.key == "Enter") {
                sendMessage();
              }
          ...
          />
          <button className='cursor-pointer bg-red-200 rounded-md px-9' onClick={sendMessage}>Send Message</button>
        </div>
      </div>
    </>
